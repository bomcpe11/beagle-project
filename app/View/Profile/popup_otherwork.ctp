<script>
	...
	function savedNewOtherwork(){
		if( validateOtherwork() ){
			loading();
			jQuery.post('<?php echo $this->Html->url('/Profile/savedNewOtherwork');?>'
	...
	function editOtherwork(){
		if( validateOtherwork() ){
			loading();
			jQuery.post('<?php echo $this->Html->url('/Profile/editOtherwork');?>'
	...
	function deleteOtherwork(id){
		jConfirm('กรุณายืนยัน เพื่อลบรายการ ', 
				function(){ //okFunc
					loading();
					jQuery.post('<?php echo $this->Html->url('/Profile/deleteOtherwork');?>'
						,{'data':{'id':id}}
						,function(data){
							unloading();
							jAlert(data.msg

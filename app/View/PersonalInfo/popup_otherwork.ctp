<script type="text/javascript">
		...
		var html = '<div id="popup_otherwork_container" style="width:500px;">\
						<table style="width:100%;">\
							...
							<tr>\
								...
								<td>\
									<input id="otherwork_yearfinish" style="width:40px" type="text" maxlength="4" value="'+yearfinish+'" '+( (id)?'disabled':'' )+'> กรุณากรอกเป็นปี พ.ศ.\
								</td>\
							</tr>\
						</table>\
					</div>';
	...
	function savedNewOtherwork(){
		if( validateOtherwork() ){
			loading();
			jQuery.post('<?php echo $this->Html->url('/PersonalInfo/savedNewOtherwork');?>'
	...
	function editOtherwork(){
		if( validateOtherwork() ){
			loading();
			jQuery.post('<?php echo $this->Html->url('/PersonalInfo/editOtherwork');?>'
	...
	function deleteOtherwork(id){
		jConfirm('กรุณายืนยัน เพื่อลบรายการ ', 
				function(){ //okFunc
					loading();
					jQuery.post('<?php echo $this->Html->url('/PersonalInfo/deleteOtherwork');?>'
						,{'data':{'id':id}}
						,function(data){
							unloading();
							jAlert(data.msg

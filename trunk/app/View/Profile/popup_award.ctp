<script>
	...
	function saveNewAward(){
		if( validateAward() ){
			loading();
			jQuery.post('<?php echo $this->Html->url('/Profile/savedNewAward');?>'
	...
	function editAward(){
		if( validateAward() ){
			loading();
			jQuery.post('<?php echo $this->Html->url('/Profile/editAward');?>'
	...
	function deletedAward(id){
		jConfirm('กรุณายืนยัน เพื่อลบรายการ ', 
				function(){ //okFunc
					loading();
					jQuery.post('<?php echo $this->Html->url('/Profile/deletedAward');?>'
						,{'data':{'id':id}}
						,function(data){
							unloading();
